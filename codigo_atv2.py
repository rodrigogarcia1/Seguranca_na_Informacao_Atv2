...
# Lista teste passado pelo professor
teste = [
...
# Lista para realizar verificação
lista = [
...
# Lista que é guardado todos os resultados de sha256 e md5, respectivamente
data = []

for i in range(len(lista)):
    aux = []
    aux.append(lista[i][0])
    aux.append(hashlib.sha256(lista[i][0].encode('utf-8')).hexdigest()) #SHA256
    aux.append(hashlib.md5(lista[i][0].encode('utf-8')).hexdigest()) #MD5

    # Verificação SHA256
    if (hashlib.sha256(lista[i][0].encode('utf-8')).hexdigest()) == (lista[i][1]):
        aux.append("OK")
    elif (hashlib.sha256(lista[i][0].encode('utf-8')).hexdigest()) != (lista[i][1]):
        aux.append("NOT OK")

    # Verificação MD5
    if (hashlib.md5(lista[i][0].encode('utf-8')).hexdigest()) == (lista[i][2]):
        aux.append("OK")
    elif (hashlib.md5(lista[i][0].encode('utf-8')).hexdigest()) != (lista[i][2]):
        aux.append("NOT OK")

    data.append(aux)

# Criar um DataFrame a partir dos dados
df = pd.DataFrame(data, columns=['Frase', 'Hash SHA256', 'Hash MD5', 'Verificacao Sha256', 'Verificacao MD5'])

# Função para adicionar a justificativa
def adicionar_justificativa(row):
    if row['Verificacao Sha256'] == 'NOT OK' and row['Verificacao MD5'] == 'NOT OK':
        return 'Frase falsa - Sha256 e MD5 estao divergentes.'
    elif row['Verificacao Sha256'] == 'NOT OK' and row['Verificacao MD5'] != 'NOT OK':
        return 'Frase falsa - Sha256 esta divergente'
    elif row['Verificacao Sha256'] != 'NOT OK' and row['Verificacao MD5'] == 'NOT OK':
        return 'Frase falsa - MD5 esta divergente'
    return 'Frase verdadeira - Conforme'

# Aplicar a função a cada linha do DataFrame
df['Justificativa'] = df.apply(adicionar_justificativa, axis=1)

# Printa dataframe
print(df)

# Salvar o DataFrame em um arquivo CSV separado por ";" --Descomentar e alterar diretório para gerar CSV
#df.to_csv('C:/Users/Admin/Desktop/Software/8ciclo/SI/atividade2.csv', sep=';', index=False)